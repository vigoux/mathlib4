...
theorem volume_normLessOne₀ :
  ...
  rw [← setLIntegral_one, Measure.volume_eq_prod, ← lintegral_indicator _ measurableSet_normLessThanOne₀,
  ...
  · exact measurableSet_normLessThanOne₁
  ...
  · refine Measurable.indicator ?_ ?_
    ...
    · let f : ({w : InfinitePlace K // IsComplex w} → ℝ) →
      ...
      · dsimp only [s]
        refine MeasurableSet.preimage measurableSet_normLessThanOne₁ ?_
        dsimp only [f]
        rw [measurable_prod]
        refine ⟨?_, ?_⟩
        · simp
...
theorem normEqOne₂_eq_image : {x | x ∈ normLessThanOne₂ K ∧
  ...
  simp_rw [Set.mem_setOf_eq, normLessThanOne₂, Set.mem_image, Set.mem_preimage, fusionEquiv_apply,
    normLessThanOne₁, Set.mem_setOf_eq, Set.mem_diff, Set.mem_preimage,
...
theorem volume_normLessThanOne :
  ...
  rw [volume_normLessThanOne_step1, volume_normLessOne₀]
  rw [← (fusionEquiv_measure_preserving K).setLIntegral_comp_preimage]
