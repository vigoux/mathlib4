...
theorem list_reverse_induction (p : List α → Prop) (base : p [])
  ...
  have pq : ∀ l, p (reverse l) → q l := by unfold_let; simp only [reverse_reverse]; intro; exact id
  have qp : ∀ l, q (reverse l) → p l := by unfold_let; simp only [reverse_reverse]; intro; exact id
  intro l
  apply qp
