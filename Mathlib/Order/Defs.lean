...
lemma eq_of_incomp {α : Sort*} {lt : α → α → Prop} [IsTrichotomous α lt] {a b} :
...
lemma incomp_iff_eq {α : Sort*} {lt : α → α → Prop} [IsTrichotomous α lt] [IsIrrefl α lt] (a b) :
    ¬lt a b ∧ ¬lt b a ↔ a = b :=
  Iff.intro eq_of_incomp fun hab => hab ▸ And.intro (irrefl_of lt a) (irrefl_of lt a)

lemma not_lt_of_lt {α : Sort*} {lt : α → α → Prop} [IsStrictOrder α lt] {a b} :
...
lemma not_lt_of_gt (h : a > b) : ¬a < b := lt_asymm h
...
def maxDefault {α : Type*} [LE α] [DecidableRel ((· ≤ ·) : α → α → Prop)] (a b : α) :=
  if a ≤ b then b else a

/-- Default definition of `min`. -/
def minDefault {α : Type*} [LE α] [DecidableRel ((· ≤ ·) : α → α → Prop)] (a b : α) :=
...
lemma lt_or_gt_of_ne (h : a ≠ b) : a < b ∨ a > b :=
  match lt_trichotomy a b with
...
lemma le_imp_le_of_lt_imp_lt {β} [Preorder α] [LinearOrder β] {c d : β}
    (H : d < c → b < a) (h : a ≤ b) : c ≤ d :=
  le_of_not_lt fun h' => not_le_of_gt (H h') h

theorem min_def (a b : α) : min a b = if a ≤ b then a else b := by rw [LinearOrder.min_def a]
theorem max_def (a b : α) : max a b = if a ≤ b then b else a := by rw [LinearOrder.max_def a]
...
theorem le_min (h₁ : c ≤ a) (h₂ : c ≤ b) : c ≤ min a b := by
  -- Porting note: no `min_tac` tactic
  if h : a ≤ b
  then simp [min_def, if_pos h]; exact h₁
...
theorem max_le (h₁ : a ≤ c) (h₂ : b ≤ c) : max a b ≤ c := by
  -- Porting note: no `min_tac` tactic
  if h : a ≤ b
  then simp [max_def, if_pos h]; exact h₂
...
@[simp] theorem min_self (a : α) : min a a = a := by simp [min_def]

theorem min_eq_left (h : a ≤ b) : min a b = a := by
  apply Eq.symm; apply eq_min (le_refl _) h; intros; assumption

theorem min_eq_right (h : b ≤ a) : min a b = b := min_comm b a ▸ min_eq_left h

theorem eq_max (h₁ : a ≤ c) (h₂ : b ≤ c) (h₃ : ∀ {d}, a ≤ d → b ≤ d → c ≤ d) :
    c = max a b :=
...
theorem max_eq_left (h : b ≤ a) : max a b = a := by
  apply Eq.symm; apply eq_max (le_refl _) h; intros; assumption

theorem max_eq_right (h : a ≤ b) : max a b = b := max_comm b a ▸ max_eq_left h

-- these rely on lt_of_lt
theorem min_eq_left_of_lt (h : a < b) : min a b = a := min_eq_left (le_of_lt h)
theorem min_eq_right_of_lt (h : b < a) : min a b = b := min_eq_right (le_of_lt h)
theorem max_eq_left_of_lt (h : b < a) : max a b = a := max_eq_left (le_of_lt h)
theorem max_eq_right_of_lt (h : a < b) : max a b = b := max_eq_right (le_of_lt h)

-- these use the fact that it is a linear ordering
theorem lt_min (h₁ : a < b) (h₂ : a < c) : a < min b c := by
  cases le_total b c <;> simp [min_eq_left, min_eq_right, *]

theorem max_lt (h₁ : a < c) (h₂ : b < c) : max a b < c := by
