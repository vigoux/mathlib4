...
section Semiring
variable [Semiring S] [CharZero S] [NNRatCast S] [Module ℚ≥0 S]
...
instance instLinearMapClass [FunLike F R S] [RingHomClass F R S] :
...
section Ring
variable [Ring S] [CharZero S] [RatCast S] [Module ℚ S]
...
lemma algebraMap_rat_rat : algebraMap ℚ ℚ = RingHom.id ℚ := rfl
