...
complex-valued random variable `f`, the $$L^{2m}$$-norm of the sum of `n` samples of `f` is bounded
by the $$L^{2m}$$-norm of `f` up to some multiplicative constant.
-/

open Finset Fintype Nat Real
variable {α : Type*} {A : Finset α} {m n : ℕ}

local notation:70 s:70 " ^^ " n:71 => Fintype.piFinset fun _ : Fin n ↦ s

private lemma step_one (hA : A.Nonempty) (f : α → ℝ) (a : Fin n → α)
...
-- double_multinomial
private lemma step_six {f : α → ℝ} {a b : Fin n → α} :
...
private lemma step_seven {f : α → ℝ} {a b : Fin n → α} :
...
private lemma step_eight {f : α → ℝ} {a b : Fin n → α} :
