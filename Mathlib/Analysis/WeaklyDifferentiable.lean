...
open Filter MeasureTheory NormedSpace Measure

noncomputable section

variable {α V W : Type*} {A : Set α}
...
instance [MeasurableSpace α] [TopologicalSpace α] {μ : Measure α} [IsLocallyFiniteMeasure μ] :
  ...
  ⟨fun x ↦ (μ.finiteAt_nhds x).mono le_rfl restrict_le_self⟩
...
def AAux (ϕ : V → ℝ) (u : SobolevSpaceAux U) : V :=
  ∫ x in U, SobolevSpaceAux.pr0 (U := U) u x •
    (InnerProductSpace.toDual ℝ _).symm (fderiv ℝ ϕ x) +
  ∫ x in U, ϕ x • SobolevSpaceAux.pr1 (U := U) u x

def A_L {ϕ : V → ℝ} (h1 : ContDiff ℝ ⊤ ϕ) (h2 : HasCompactSupport ϕ) :
...
def SobolevSpace : Submodule ℝ (SobolevSpaceAux U) :=
  ⨅ (ϕ : V → ℝ), ⨅ (h1 : ContDiff ℝ ⊤ ϕ), ⨅ (h2 : HasCompactSupport ϕ), LinearMap.ker (A_L h1 h2)
