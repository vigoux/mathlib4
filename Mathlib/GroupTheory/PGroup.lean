...
theorem card_modEq_card_fixedPoints [Finite (fixedPoints G α)] :
    Nat.card α ≡ Nat.card (fixedPoints G α) [MOD p] := by
...
theorem nonempty_fixed_point_of_prime_not_dvd_card [Finite (fixedPoints G α)]
    (hpα : ¬ p ∣ Nat.card α) : (fixedPoints G α).Nonempty :=
