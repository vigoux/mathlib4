...
@[simp] lemma Finset.inter_inv {A B : Finset G} : (A ∩ B)⁻¹ = A⁻¹ ∩ B⁻¹ := coe_injective <| by simp

lemma Set.inv_smul_finset {a : G} {A : Set G} : (a • A)⁻¹ = op a⁻¹ • A⁻¹ := by
  ext x; simp [mem_smul_set_iff_inv_smul_mem]

lemma Finset.inv_smul_finset {a : G} {A : Finset G} : (a • A)⁻¹ = op a⁻¹ • A⁻¹ :=
  coe_injective <| by simp [Set.inv_smul_finset]

lemma Finset.mem_inv' {G : Type*} [DecidableEq G] [InvolutiveInv G] {a : G} {A : Finset G} :
    a ∈ A⁻¹ ↔ a⁻¹ ∈ A := by
  rw [←mem_coe, coe_inv, Set.mem_inv, mem_coe]
  ...
  mul_mem' := by
    ...
    have ⟨t, ht⟩ : ((A ∩ (c * d) • A) ∩ (A ∩ (a * b)⁻¹ • A)).Nonempty := by
      ...
      have : (((A ∩ (c * d) • A) ∪ (A ∩ (a * b)⁻¹ • A)).card : ℚ) ≤ A.card := by
        rw [Nat.cast_le, ←inter_distrib_left]
        exact card_le_of_subset (inter_subset_left _ _)
      linarith
    ...
    exact ⟨a * b * t, ((c * d)⁻¹ * t)⁻¹, by simp [ht, mul_assoc]⟩
...
lemma subgroup_strong_bound_left (h : (A * A).card < (3 / 2 : ℚ) * A.card) (a : G) (ha : a ∈ A) :
  ...
  have h₁ : (A⁻¹ * A) * (A⁻¹ * A) = A⁻¹ * A := by
    rw [←Finset.coe_inj, coe_mul, coe_mul, ←symmetricSubgroup_coe _ h, Subgroup.mul_self]
...
lemma subgroup_strong_bound_right (h : (A * A).card < (3 / 2 : ℚ) * A.card) (a : G) (ha : a ∈ A) :
  ...
  have lr : l ∪ r ⊆ a • (A⁻¹ * A) := by
    ...
    exact ⟨A_subset_aH a ha, inter_subset_left _ _⟩
