...
theorem specializes_TFAE (x y : X) :
  ...
  tfae
    1 → 2 := (pure_le_nhds _).trans
    2 → 3 := fun h s hso hy => h (hso.mem_nhds hy)
    3 → 4 := fun h s hsc hx => of_not_not fun hy => h sᶜ hsc.isOpen_compl hy hx
    4 → 5 := fun h => h _ isClosed_closure (subset_closure <| mem_singleton _)
    6 ↔ 5 := isClosed_closure.closure_subset_iff.trans singleton_subset_iff
    5 ↔ 7 := by
